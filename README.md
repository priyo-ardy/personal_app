...
5. Pada file `.env` tambahkan baris `phone_salt` dan `email_salt` untuk `hashing email dan nomor telepon user`, `npwp_salt` untuk hashing NPWP, `bank_account_no_salt` untuk hasing Bank account, `ktp_salt` untuk hasing nomor KTP, `bpjs_kesehatan_hash` untuk hasing BPJS Kesehatan, `bpjs_ketenagakerjaan_salt` untuk hashing BPJS ketenagakerjaan
