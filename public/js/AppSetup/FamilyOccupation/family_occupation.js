window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/family_occupation/table");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/family_occupation/save",
        "POST",
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/family_occupation/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/family_occupation/update",
        "POST",
        new FormData(formData),
      )
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/family_occupation/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/family_occupation/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "family_occupation_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
