window.onload = () => {
  loadTable("dataTable", "/employee_rank/table");
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/employee_rank/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/employee_rank/get/" + token, "GET")
      .then((result) => {
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/employee_rank/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/employee_rank/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "employee_rank_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
