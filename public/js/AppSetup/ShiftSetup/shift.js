...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/shift_setup/get/" + token, "GET")
      .then((result) => {
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/shift_setup/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "shift_setup_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/shift_setup/mass-delete", selectedData);
    } catch (e) {
      pesanError(e.message);
      hideLoading();
    }
