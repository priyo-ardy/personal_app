window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/city/table");
...
const inputForm = {
  ...
  province: document.getElementById("data_province"),
...
function resetForm() {
  ...
  $(inputForm.province).trigger("change");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/city/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/city/get/" + token, "GET")
      .then((result) => {
        hideLoading();
        inputForm.token.value = result.data.token;
        inputForm.code.value = result.data.code;
        inputForm.province.value = result.data.province;
        $(inputForm.province).trigger("change");
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/city/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/city/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/city/export", {
      method: "GET",
    ...
    a.href = url;
    a.download = "city_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
