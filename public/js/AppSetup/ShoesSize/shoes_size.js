window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/shoes_size/table");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/shoes_size/save", "POST", new FormData(formData))
        .then((result) => {
          resetForm();
          refreshTable();
          hideLoading();
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/shoes_size/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/shoes_size/update", "POST", new FormData(formData))
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/shoes_size/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/shoes_size/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "shoes_size_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
