window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/department/table");
...
function resetForm() {
  ...
  $(inputForm.effective_date).trigger("change");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/department/save", "POST", new FormData(formData))
        .then((result) => {
          resetForm();
          refreshTable();
          hideLoading();
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/department/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/department/update", "POST", new FormData(formData))
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/department/mass-delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/department/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "department_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
