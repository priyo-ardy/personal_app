window.onload = () => {
  loadTable("dataTable", "/employee_class_nbhx/table");
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/employee_class_nbhx/save",
        "POST",
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/employee_class_nbhx/get/" + token, "GET")
      .then((result) => {
...
buttons.update.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/employee_class_nbhx/update",
        "POST",
        new FormData(formData)
      )
        .then((result) => {
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/employee_class_nbhx/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/employee_class_nbhx/export", {
      method: "GET",
