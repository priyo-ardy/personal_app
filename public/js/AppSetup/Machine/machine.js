...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/machine/get/" + token, "GET")
      .then((result) => {
        window.location.replace(baseurl + "/machine/show/" + result.data);
      })
      .catch((err) => {
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    a.href = url;
    a.download = "machine_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/machine/mass-delete", selectedData);
    } catch (e) {
      pesanError(e.message);
      hideLoading();
    }
