window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/job_data_action/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/job_data_action/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/job_data_action/update",
        "POST",
        new FormData(formData),
      )
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/job_data_action/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/job_data_action/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "job_data_action_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
