...
const inputForm = {
  ...
  symbol: document.getElementById("data_symbol"),
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/uom/save", "POST", new FormData(formData))
        .then((result) => {
...
buttons.update.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/uom/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/uom/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/uom/export", {
      method: "GET",
    ...
    a.href = url;
    a.download = "uom_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
