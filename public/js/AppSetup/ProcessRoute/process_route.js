window.onload = () => {
  loadTable("dataTable", "/process_routes/table");
...
const inputForm = {
  ...
  process_route: document.getElementById("data_route"),
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/process_routes/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/process_routes/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "process_route_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
