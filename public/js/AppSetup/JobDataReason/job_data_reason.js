window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/job_data_reason/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/job_data_reason/get/" + token, "GET")
      .then((result) => {
        hideLoading();
        inputForm.token.value = result.data.token;
        inputForm.code.value = result.data.code;
        inputForm.action.value = result.data.action;
        $(inputForm.action).trigger("change");
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/job_data_reason/update",
        "POST",
        new FormData(formData),
      )
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/job_data_reason/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/job_data_reason/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "job_data_reason_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
