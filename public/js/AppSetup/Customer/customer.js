window.onload = () => {
  loadTable("dataTable", "/customer/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/customer/get/" + token, "GET")
      .then((result) => {
        window.location.replace(baseurl + "/customer/show/" + result.data);
      })
      .catch((err) => {
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/customer/mass-delete", selectedData);
    } catch (e) {
      pesanError(e.message);
      hideLoading();
    }
