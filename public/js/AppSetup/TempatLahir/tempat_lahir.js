window.onload = () => {
  loadTable("dataTable", "/birth_place/table");
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/birth_place/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/birth_place/get/" + token, "GET")
      .then((result) => {
...
buttons.update.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/birth_place/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/birth_place/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/birth_place/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "birth_place_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
