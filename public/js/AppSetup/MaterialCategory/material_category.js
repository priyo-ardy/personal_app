window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/material_category/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/material_category/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/material_category/update",
        "POST",
        new FormData(formData)
      )
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/material_category/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
