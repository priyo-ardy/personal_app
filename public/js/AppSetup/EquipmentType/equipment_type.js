window.addEventListener("DOMContentLoaded", () => {
    loadTable("dataTable", "/equipment_type/table");
...
function getData(token) {
    try {
        loading();
        fetchData(baseurl + "/equipment_type/get/" + token, "GET")
            .then((result) => {
                hideLoading();
...
button.delete.addEventListener("click", (e) => {
    ...
    if (!isAnyCheckboxChecked) {
        ...
        try {
            disableData("/equipment_type/delete", selectedData);
        } catch (e) {
            pesanError(e.message);
        }
    }
...
button.export.addEventListener("click", async (e) => {
    try {
        ...
        const response = await fetch(baseurl + "/equipment_type/export", {
            method: "GET",
        ...
        a.href = url;
        a.download =
            "equipment_type_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
