...
window.onload = () => {
  loadTable("dataTable", "/team_leader/table");
  loadEmployee();
...
buttons.delete.addEventListener("click", async () => {
  ...
  try {
    ...
    const disable = await disableData("/team_leader/delete", selectedData);
...
buttons.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/team_leader/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "team_leader_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/team_leader/save", "POST", new FormData(formData))
        .then((result) => {
...
function loadEmployee() {
  ...
  fetchData(baseurl + "/team_leader/employee_list", "GET")
    .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/team_leader/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
buttons.update.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/team_leader/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
