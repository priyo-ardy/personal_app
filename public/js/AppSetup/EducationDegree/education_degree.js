window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/education_degree/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/education_degree/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/education_degree/update",
        "POST",
        new FormData(formData),
      )
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/education_degree/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/education_degree/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "education_degree_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
