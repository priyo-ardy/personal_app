window.addEventListener("DOMContentLoaded", () => {
    loadTable("dataTable", "/employee_facility/table");
...
button.save.addEventListener("click", () => {
    if (validasi()) {
        try {
            loading();
            fetchData(
                baseurl + "/employee_facility/save",
                "POST",
...
function getData(token) {
    try {
        loading();
        fetchData(baseurl + "/employee_facility/get/" + token, "GET")
            .then((result) => {
                hideLoading();
...
button.update.addEventListener("click", (e) => {
    if (validasi()) {
        try {
            loading();
            fetchData(
                baseurl + "/employee_facility/update",
                "POST",
                new FormData(formData),
            )
                .then((result) => {
...
button.delete.addEventListener("click", (e) => {
    ...
    if (!isAnyCheckboxChecked) {
        ...
        try {
            disableData("/employee_facility/delete", selectedData);
        } catch (e) {
            pesanError(e.message);
        }
    }
...
button.export.addEventListener("click", async (e) => {
    try {
        ...
        const response = await fetch(baseurl + "/employee_facility/export", {
            method: "GET",
        ...
        a.href = url;
        a.download =
            "employee_facility_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
