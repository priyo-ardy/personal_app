window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/factory/table");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/factory/save", "POST", new FormData(formData))
        .then((result) => {
          resetForm();
          refreshTable();
          hideLoading();
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/factory/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/factory/update", "POST", new FormData(formData))
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/factory/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/factory/export", {
      method: "GET",
    ...
    a.href = url;
    a.download = "factory_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
