window.onload = () => {
  loadTable("dataTable", "/employee_grade/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/employee_grade/get/" + token, "GET")
      .then((result) => {
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/employee_grade/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/employee_grade/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "employee_grade_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
