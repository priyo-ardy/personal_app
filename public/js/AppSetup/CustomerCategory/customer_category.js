window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/customer_category/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/customer_category/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/customer_category/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    a.href = url;
    a.download =
      "customer_category_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
