window.onload = () => {
  loadTable("dataTable", "/nbhx_position/table");
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/nbhx_position/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/nbhx_position/get/" + token, "GET")
      .then((result) => {
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/nbhx_position/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/nbhx_position/export", {
      method: "GET",
