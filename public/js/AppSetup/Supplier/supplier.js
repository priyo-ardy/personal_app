window.onload = () => {
  loadTable("dataTable", "/supplier/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/supplier/get/" + token, "GET")
      .then((result) => {
        window.location.replace(baseurl + "/supplier/show/" + result.data);
      })
      .catch((err) => {
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    a.href = url;
    a.download =
      "supplier_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/supplier/mass-delete", selectedData);
    } catch (e) {
      pesanError(e.message);
      hideLoading();
    }
