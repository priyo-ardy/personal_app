window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/province/table");
...
const inputForm = {
  ...
  country: document.getElementById("data_country"),
...
function resetForm() {
  ...
  $(inputForm.country).trigger("change");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/province/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/province/get/" + token, "GET")
      .then((result) => {
        hideLoading();
        inputForm.token.value = result.data.token;
        inputForm.code.value = result.data.code;
        inputForm.country.value = result.data.country;
        $(inputForm.country).trigger("change");
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/province/update", "POST", new FormData(formData))
        .then((result) => {
          pesanSukses(result.message);
          resetForm();
          refreshTable();
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/province/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/province/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "province_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
