window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/uniform_size/table");
...
button.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/uniform_size/save", "POST", new FormData(formData))
        .then((result) => {
          resetForm();
          refreshTable();
          hideLoading();
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/uniform_size/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/uniform_size/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/uniform_size/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "uniform_size_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
