window.onload = () => {
  loadTable("dataTable", "/workshop/table");
...
buttons.save.addEventListener("click", () => {
  if (validasi()) {
    try {
      loading();
      fetchData(baseurl + "/workshop/save", "POST", new FormData(formData))
        .then((result) => {
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/workshop/get/" + token, "GET")
      .then((result) => {
...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/workshop/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
