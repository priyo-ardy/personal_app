window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/family_relation/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/family_relation/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.update.addEventListener("click", (e) => {
  if (validasi()) {
    try {
      loading();
      fetchData(
        baseurl + "/family_relation/update",
        "POST",
        new FormData(formData),
      )
        .then((result) => {
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/family_relation/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/family_relation/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "family_relation_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
