window.addEventListener("DOMContentLoaded", () => {
  loadTable("dataTable", "/absence_status/table");
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/absence_status/get/" + token, "GET")
      .then((result) => {
        hideLoading();
...
button.delete.addEventListener("click", (e) => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/absence_status/delete", selectedData);
    } catch (e) {
      pesanError(e.message);
    }
  }
...
button.export.addEventListener("click", async (e) => {
  try {
    ...
    const response = await fetch(baseurl + "/absence_status/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "absence_status_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
