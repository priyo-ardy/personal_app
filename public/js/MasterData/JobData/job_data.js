...
buttons.delete.addEventListener("click", () => {
  ...
  if (!isAnyCheckboxChecked) {
    ...
    try {
      disableData("/job_data/mass-delete", selectedData);
    } catch (e) {
...
buttons.export.addEventListener("click", async () => {
  try {
    ...
    const response = await fetch(baseurl + "/job_data/export", {
      method: "GET",
    ...
    a.href = url;
    a.download =
      "latest_job_data_list_" + moment().format("YYYYMMDD_HHMMSS") + ".xlsx";
...
function getData(token) {
  try {
    loading();
    fetchData(baseurl + "/job_data/get/" + token, "GET")
      .then((result) => {
        window.location.replace(baseurl + "/job_data/show/" + result.data);
      })
      .catch((err) => {
