    ...
    public function prosesLogin(array $data)
    {
        try {
            ...
            if ($cek_user->login_attempts >= 3) { // jika user salah memasukkan password lebih dari 3 kali kembalikan error user terkunci
                log_message('error', "User {user} is locked", ['user' => $data['user_name']]);
            ...
            if ($cek_user->user_status == 'inactive') { // jika user tidak aktif kembalikan error
                log_message('error', "User {user} is inactive", ['user' => $data['user_name']]);
            ...
            if (!$verify) { //Jika password yang dimasukkan salah
                ...
                log_message('error', 'User {user} failed to login from {ip}', ['user' => $data['user_name'], 'ip' => $_SERVER['REMOTE_ADDR']]);
            ...
            log_message('error', '[AuthService::prosesLogin] Unexpected error occured : {err} from {ip}', ['err' => $e->getMessage(), 'ip' => $_SERVER['REMOTE_ADDR']]);
            throw $e;
        }
    }
}
