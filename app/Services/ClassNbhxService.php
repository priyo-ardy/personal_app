...
use App\Validation\ClassNbhxValidation;
...
class ClassNbhxService
{
    ...
    public function __construct(ClassNbhxRepository $classRepo)
    {
    ...
    {
        try {
            $this->validasi->setRules(ClassNbhxValidation::$save);

            if ($this->validasi->run($data) === false) {
                $error_to_string = implode("<br>", $this->validasi->getErrors());
                log_message(
                    'error',
                    '[ClassNbhxService::save] Validation failed by {NIK} from {ip} with error : {err}',
                    [
            ...
            if ($this->db->transStatus() === false) {
                ...
                log_message(
                    'error',
                    '[ClassNbhxService::save] save failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $this->db->error()
    ...
    {
        try {
            $result = $this->classRepo->findData($id);
            if (!$result) {
                log_message(
                    'error',
                    '[ClassNbhxService::getData] getData failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => 'Data not found'
    ...
    {
        try {
            $this->validasi->setRules(ClassNbhxValidation::$update);

            if ($this->validasi->run($data) === false) {
                $error_to_string = implode("<br>", $this->validasi->getErrors());
                log_message(
                    'error',
                    '[ClassNbhxService::updateData] Validation failed by {NIK} from {ip} with error : {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $error_to_string
            ...
            $this->db->transStart();
            $this->classRepo->updateData($id, $data);
            $this->db->transComplete();
            ...
            if ($this->db->transStatus() === false) {
                ...
                log_message(
                    'error',
                    '[ClassNbhxService::updateData] updateData failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $this->db->error()
    ...
    public function delete($data)
    {
        try {
            $this->db->transStart();
            $this->classRepo->deleteData($data);
            $this->db->transComplete();
            ...
            if ($this->db->transStatus() === false) {
                ...
                log_message(
                    'error',
                    '[ClassNbhxService::delete] delete failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $this->db->error()
    ...
    public function loadTable($requestedData)
    {
        try {
            ...
            log_message('error', '[ClassNbhxService::loadTable] Unexpexted error occured : {err} from {ip}', ['err' => $e->getMessage(), 'ip' => $_SERVER['REMOTE_ADDR']]);
            return $e;
        }
    }
    ...
    {
        try {
            $fileName = "employee_class_nbhx_list" . date("Ymd_his") . '.xlsx';
            ...
            $dataCallback = function ($offset, $limit) {
                ...
                return $this->classRepo->chunkedData($offset, $limit, 'code', $column);
            };

            return export_to_excel($fileName, $headers, $dataCallback);
        } catch (\Exception $e) {
            log_message('error', '[ClassNbhxService::export] Unexpexted error occured : {err} from {ip}', ['err' => $e->getMessage(), 'ip' => $_SERVER['REMOTE_ADDR']]);
            return $e;
        }
    }
}
