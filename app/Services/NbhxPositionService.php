...
use App\Validation\NbhxPositionValidation;
use App\Repositories\DataTableRepository;
use App\Models\AppSetup\NbhxPosition\NbhxPositionModel;
use App\Traits\ResponseTrait;
use CodeIgniter\HTTP\ResponseInterface;
use Config\Services;
use Config\Database;

class NbhxPositionService
{
    use ResponseTrait;
    protected $db;
    protected $validasi;
    ...
    public function __construct(NbhxPositionRepository $nbhxPosition)
    {
    ...
    {
        try {
            $this->validasi->setRules(NbhxPositionValidation::$save);

            if ($this->validasi->run($data) === false) {
                $error_to_string = implode("<br>", $this->validasi->getErrors());
                log_message(
                    'error',
                    '[NbhxPositionService::save] Validation failed by {NIK} from {ip} with error : {err}',
                    [
            ...
            $this->db->transStart();
            $this->positionRepo->saveData($data);
            ...
            if ($this->db->transStatus() === false) {
                ...
                log_message(
                    'error',
                    '[NbhxPositionService::save] save failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $this->db->error()
                ...
                throw new \Exception("Failed to save NBHX position category data", ResponseInterface::HTTP_INTERNAL_SERVER_ERROR);
            }
    ...
    {
        try {
            $result = $this->positionRepo->findData($id);
            if (!$result) {
                log_message(
                    'error',
                    '[NbhxPositionService::getData] getData failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => 'Data not found'
    ...
    {
        try {
            $this->validasi->setRules(NbhxPositionValidation::$update);

            if ($this->validasi->run($data) === false) {
                $error_to_string = implode("<br>", $this->validasi->getErrors());
                log_message(
                    'error',
                    '[NbhxPositionService::updateData] Validation failed by {NIK} from {ip} with error : {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $error_to_string
            ...
            $this->db->transStart();
            $this->positionRepo->updateData($id, $data);
            $this->db->transComplete();
            ...
            if ($this->db->transStatus() === false) {
                ...
                log_message(
                    'error',
                    '[NbhxPositionService::updateData] updateData failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $this->db->error()
                ...
                throw new \Exception("Failed to update NBHX position category", ResponseInterface::HTTP_INTERNAL_SERVER_ERROR);
            }
    ...
    public function delete($data)
    {
        try {
            $this->db->transStart();
            $this->positionRepo->deleteData($data);
            $this->db->transComplete();
            ...
            if ($this->db->transStatus() === false) {
                ...
                log_message(
                    'error',
                    '[NbhxPositionService::delete] delete failed by {NIK} from {ip} with error {err}',
                    [
                        'NIK' => session()->get('user_name'),
                        'ip' => $_SERVER['REMOTE_ADDR'],
                        'err' => $this->db->error()
                ...
                throw new \Exception("Failed to delete NBHX position category", ResponseInterface::HTTP_INTERNAL_SERVER_ERROR);
            }
